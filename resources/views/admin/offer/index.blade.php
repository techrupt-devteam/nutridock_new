                  ...
                  @foreach($data as $key=>$value)
                    <tr>
                      ...
                      <td>
                        <table>
                           <tr>
                              <form data-parsley-validate="parsley"  action="{{ url('/admin')}}/update_order_no1/{{$value['id']}}" method="POST">
                                <td>
                      ...
                      <td>
                        ...
                        <a class="fancybox btn btn-sm btn-success" rel="group" href="{{url('/')}}/uploads/offer/{{$value['image']}}" title="View"><i class="fa fa-eye"></i></a>
...
<script src="//cdnjs.cloudflare.com/ajax/libs/fancybox/2.1.5/jquery.fancybox.min.js"></script>
  ...
  <script type="text/javascript">
    ...
    {  
      ...
      swal({
        title: "Offer status",
        text: "Are You sure to change offer status",
        icon: "warning",
          buttons: [
            'Cancel',
            'Yes, change it!'
          ...
          { 
            ...
            //alert(status);
             $.ajax({
                  url: "{{url('/admin')}}/status_offer",
                  type: 'post',
                  data: {status:status,plan_ids:plan_id},
                  success: function (data) 
                  {
                    swal("Success", "Offer status successfully changed !", "success");
                  }
              });
